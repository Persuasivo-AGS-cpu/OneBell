...
function renderOnboarding() {
    if (onboardingStep === 0) {
        // Step 1 - Welcome
        mainContent.innerHTML = '<div class="onboarding-container" style="justify-content: flex-end; padding-bottom: 40px; background: linear-gradient(180deg, rgba(0,0,0,0.1) 0%, var(--bg-color) 90%), url(./assets/onboarding_hero_40yo.png) center center/cover no-repeat; border-radius: 0 0 36px 36px; position: relative;">' +
    ...
    if (onboardingStep === 1) { 
        ...
        stepContent = '<h2 class="onboarding-title" style="margin-bottom: 32px;">What kettlebell weight do you have?</h2>' +
            ...
            '</div>' +
            '</div>' +
            (state.userProfile.isCustomWeight ? '<div style="margin-top: 24px;"><input type="number" id="custom-weight-input" placeholder="Weight in kg" value="' + (state.userProfile.weight || '') + '" class="input-base text-center"></div>' : '');
        ...
        mainContent.innerHTML = '<div class="loading-screen h-screen flex-col flex-center">' +
            '<div class="ai-orb-container">' +
                '<div class="orb-ring"></div>' +
                '<div class="orb-ring"></div>' +
                '<div class="orb-ring"></div>' +
                '<div class="orb-core"><i data-lucide="zap" style="width: 42px; height: 42px; color: var(--accent-color);"></i></div>' +
            '</div>' +
            '<h2 id="wow-title" style="font-size: 24px; font-weight: 800; color: var(--text-primary); margin-bottom: 8px; text-align:center; padding: 0 20px; min-height: 32px; transition: opacity 0.3s; letter-spacing:-0.4px;">Synthesizing Protocol...</h2>' +
...
function renderHome() {
    ...
    let heroCardHTML = '<div class="workout-card card" style="' + (programData.theme_color ? 'background: linear-gradient(145deg, '+programData.theme_color+' 0%, #000 100%);' : '') + ' border-radius: var(--border-radius-xl); padding: 24px; position: relative; overflow: hidden; box-shadow: var(--shadow-lg); margin-bottom: 32px;">' +
...
function renderProgram() {
    ...
    let todayWorkoutHTML = '<h3 style="font-size: 20px; font-weight: 800; color: var(--primary-color); margin-bottom: 16px;">Hoy</h3>' +
        '<div class="workout-card card" style="' + (programData.theme_color ? 'background: linear-gradient(145deg, '+programData.theme_color+' 0%, #000 100%);' : '') + ' border-radius: var(--border-radius-xl); padding: 24px; position: relative; overflow: hidden; box-shadow: var(--shadow-lg); margin-bottom: 32px;">' +
        '<div style="position: absolute; top: -40px; right: -40px; width: 160px; height: 160px; background: var(--accent-color); opacity: 0.15; filter: blur(40px); border-radius: 50%; pointer-events: none;"></div>' +
        '<h2 style="font-size: 28px; font-weight: 800; line-height: 1.1; color: #FFF; position: relative; margin-bottom: 12px; letter-spacing: -0.5px;">' + todaysWorkout.title + '</h2>' +
        '<div style="display: flex; gap: 8px; margin-bottom: 24px; position: relative;">' +
            '<span style="background: rgba(255,255,255,0.15); padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; color: #FFF;">' + todaysWorkout.duration + ' min</span>' +
